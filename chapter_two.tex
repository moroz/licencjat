...
\footnote{Podstawa umysłu (心地 \pinyin{xīndì}) została zdefiniowana w tekście \textit{Zutang ji} (祖堂集 \pinyin{Zǔtáng jí}), w biografii Nanyue Huairanga (南嶽懷讓 \pinyin{Nányuè Huáiràng}): “汝学心地法门，犹如下种。我说法要，譬彼天泽。汝缘合故，当见于道。”
...
\section{Buddyjska teoria poznania}
W sekcjach 45. i 46. \textit{Sutry platformy} przekazano nauki dotyczące buddyjskiej teorii poznania: o trzech kategoriach (三科 \pinyin{sān kē}) i trzydziestu sześciu konfrontacjach aktywności (動三十六對 \pinyin{dòng sānshíliù duì}). % 三科法門
Zostały one wygłoszone przez Szóstego Patriarchę dla dziesięciu najbliższych uczniów: Fahai, Zhicheng, Fada, Zhichang, Zhitong (志通 \nazwisko{Zhìtōng}), Zhiche (志徹 \nazwisko{Zhìchè}), Zhidao (志道 \nazwisko{Zhìdào}), Fazhen (法珍 \nazwisko{Fǎzhēn}), Faru i Shenhui.
Huineng powiedział im, że powodem, dla którego przekazuje im te wyjaśnienia jest fakt, iż po jego śmierci to na nich spoczywać będzie obowiązek nauczania przyszłych pokoleń.
Nauczał też, że kiedy ktoś zada im jakieś pytanie, powinni udzielić temu komuś odpowiedzi w sposób symetryczny, za pomocą konfrontacji.
...
\if 0
Następnie Szósty Patriarcha wyliczył ,,trzy kategorie'', składają się na nie: pięć skandh%
\footnote{Pięć skandh (skt. \textit{pañca skandha}, 五蘊 \pinyin{wǔ yùn} w \textit{Sutrze platformy} zapisywane 五蔭 \pinyin{wǔ yìn}), czasem tłumaczone jako ,,pięć skupisk'' --- forma materialna (skt. \textit{rūpa}, chiń. 色 \pinyin{sè}), uczucia (skt. \textit{vedanā}, chiń. 受 \pinyin{shòu}), percepcja (skt. \textit{sa\d{m}jñā}, chiń. 想 \pinyin{xiǎng}), formacje mentalne (skt. \textit{sa\d{m}skāra}, chiń. 行 \pinyin{xíng}), świadomość (skt. \textit{vijñāna}, chiń. 識 \pinyin{shí})}, osiemnaście dhatu%
\footnote{Osiemnaście dhatu (skt. 十八界 shíbā jiè} oraz dwanaście podstaw zmysłowych%
\footnote{ayatana 十二入 shíèr rù}.
\fi

Następnie Szósty Patriarcha wyjaśnił określenie ,,trzy kategorie''.
...
Na osiemnaście dhātu składają się: sześć zewnętrznych podstaw zmysłowych (\textit{bāhya-āyatana} chiń. 六塵) --- formy (skt. \textit{rūpa-āyatana}, chiń.色塵 \pinyin{sè chén}), słuch (skt. \textit{śabda-āyatana}, chiń. 聲塵 \pinyin{shēng chén}), zapach (skt. \textit{gandha-āyatana}, chiń. 香塵 \pinyin{xiāng chén}), smak (skt. \textit{rasa-āyatana}, chiń. 味塵 \pinyin{wèi chén}), dotyk (skt. \textit{spra\d{s}\d{t}avya-āyatana}, chiń. 觸塵 \pinyin{chù chén}) i idee (skt. \textit{dharma-āyatana}, chiń. 法塵 \pinyin{fǎ chén}); odpowiadające im sześć wewnętrznych podstaw zmysłowych (\textit{adhyātma-āyatana} chiń. 六門 \pinyin{liù mén}) --- oczy (skt. \textit{眼門}, chiń.  \pinyin{yǎn mén}), uszy (skt. \textit{śrota-indriya-āyatana}, chiń. 耳門 \pinyin{ěr mén}), nos (skt. \textit{ghrā\d{n}a-indriya-āyatana}, chiń. 鼻門 \pinyin{}), język (skt. \textit{jihvā-indriya-āyatana}, chiń. 舌門 \pinyin{shé mén}), ciało (skt. \textit{kaya-indriya-āyatana}, chiń. 身門 \pinyin{shēn mén}) i umysł (skt. \textit{mano-indriya-āyatana}, chiń. 意門 \pinyin{yì mén}); a także powstałe z nich sześć świadomości zmysłów (skt. \textit{vijñāna}, chiń. 六識 \pinyin{liù shí}) --- widzenie (眼識 \pinyin{yǎn shí}), słyszenie (耳識 \pinyin{ěr shí}), węch (鼻識 \pinyin{bí shí}), smak (舌識 \pinyin{shé shí}), dotyk (身識 \pinyin{shēn shí}) i myślenie (意識 \pinyin{yì shí}).
